...
def is_player1_win(handpair):
    # parse cards and divide to the players
...
# and if handX is a draw with handY then get_score(handX) = get_score(handY)
...
def get_score(hand):
    ...
    # encode the hand type in the top, then encode up to 5 cards in big-endian (4 each).
    if straighthighrank != -1 and flushsuit != -1:
        ...
        return 6 << 20 | bestcards  # fFull house
    elif flushsuit != -1:
...
# encode 5 card ranks into 20, starting with the most frequent cards,
# breaking ties by highest rank.___example: the set of ranks {5,5,T,8,T} is encoded as
...
#  takes into account the fact that ACE can be rank 0  or rank 13 -> value immediately after king. (ace face value 1)
...
# card is a 2-letter string like "2H" -> "two of hearts".
